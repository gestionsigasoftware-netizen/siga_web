-- SIGA - Salud de datos "premium": agrega consistencia de
-- bautizado/sellado a resumen_salud_datos_distrital/_nacional.
--
-- bautizado y sellado_espiritu_santo son `boolean not null default
-- false` (ver feligresia.sql, hitos_espirituales.sql) -- a diferencia
-- de fecha_nacimiento/genero/telefono, un `false` ahi NO es
-- necesariamente un dato faltante (puede ser real: la persona
-- legitimamente no esta bautizada todavia). Lo que si es una
-- inconsistencia real y accionable es bautizado=true con
-- fecha_bautismo null (dice que esta bautizado pero no sabe cuando) --
-- mismo patron para sellado_espiritu_santo/fecha_sellado. Esa es la
-- metrica que se agrega, no un "% con bautizado".
--
-- Repetible (drop + create), mismo patron que el resto de la sesion --
-- no se edita salud_datos.sql (ya ejecutado en produccion).
...
returns table (
  ...
  bautizados bigint,
  bautizados_sin_fecha bigint,
  sellados bigint,
  sellados_sin_fecha bigint
)
language sql stable security invoker set search_path = public as $$
  select
    ...
    count(p.id) filter (where p.estado_membresia = 'activo' and p.bautizado),
    count(p.id) filter (where p.estado_membresia = 'activo' and p.bautizado and p.fecha_bautismo is null),
    count(p.id) filter (where p.estado_membresia = 'activo' and p.sellado_espiritu_santo),
    count(p.id) filter (where p.estado_membresia = 'activo' and p.sellado_espiritu_santo and p.fecha_sellado is null)
...
language sql stable security invoker set search_path = public as $$
  select
    ...
    coalesce((select count(*) from personas p join congregaciones c on c.id = p.congregacion_id where c.distrito_id = d.id and c.id in (select mis_congregaciones()) and p.estado_membresia = 'activo' and p.bautizado), 0),
    coalesce((select count(*) from personas p join congregaciones c on c.id = p.congregacion_id where c.distrito_id = d.id and c.id in (select mis_congregaciones()) and p.estado_membresia = 'activo' and p.bautizado and p.fecha_bautismo is null), 0),
    coalesce((select count(*) from personas p join congregaciones c on c.id = p.congregacion_id where c.distrito_id = d.id and c.id in (select mis_congregaciones()) and p.estado_membresia = 'activo' and p.sellado_espiritu_santo), 0),
    coalesce((select count(*) from personas p join congregaciones c on c.id = p.congregacion_id where c.distrito_id = d.id and c.id in (select mis_congregaciones()) and p.estado_membresia = 'activo' and p.sellado_espiritu_santo and p.fecha_sellado is null), 0)
  from distritos d
