...
- **`supabase/distrital/cargos_distritales.sql`** (nuevo): tabla `cargos_distritales`
...
1. `supabase/catalogos/genero_personas.sql` (si no se ha ejecutado ya).
2. `supabase/reportes/resumen_nacional.sql` (si no se ha ejecutado ya).
3. `supabase/distrital/cargos_distritales.sql` (nuevo).
