...
`notificar_asignacion_acceso()` (`supabase/modulos/notificaciones.sql`) creaba la
...
- **Acción del usuario**: volver a ejecutar `supabase/modulos/notificaciones.sql`
  (es `create or replace function`, no requiere `drop` antes).
- Para la notificación que ya quedó guardada con el link viejo:
  `supabase/modulos/fix_enlace_notificaciones_acceso.sql` (ejecutar una sola vez).
...
- **Acción del usuario**: redesplegar la función —
  ...
  documentado en `docs/arquitectura/despliegue-operacion.md`).
...
1. Ejecutar `supabase/modulos/notificaciones.sql` de nuevo.
2. Ejecutar `supabase/modulos/fix_enlace_notificaciones_acceso.sql` (una sola vez).
