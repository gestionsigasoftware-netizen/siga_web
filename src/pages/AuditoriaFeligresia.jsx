...
export default function AuditoriaFeligresia() {
  ...
  useEffect(() => {
    if (!rolPrincipal || !ADMIN_LEVELS.includes(rolPrincipal.nivel)) return
    async function load() {
      ...
      if (result.error) setError(`No se pudo cargar la auditoría: ${result.error.message}`)
      setEntries(result.data ?? []); setTotal(result.count ?? 0); setLoading(false)
    }
    load()
  }, [rolPrincipal, entity, action, page])
