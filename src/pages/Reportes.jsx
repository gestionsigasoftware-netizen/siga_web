...
export default function Reportes() {
  ...
  return <div className="flex flex-col gap-6">
    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.16em] text-accent mb-2">Lectura de datos</p><h1 className="text-2xl font-semibold">Reportes</h1><p className="text-sm text-secondary mt-1">Convierte los registros de la PWA en señales para actuar.</p></div><button className="btn-secondary" disabled title="Disponible próximamente"><Download className="w-4 h-4" /> Exportar próximamente</button></div>
    ...
    <div className="grid sm:grid-cols-3 gap-3"><div className="stat-tile"><p className="text-sm text-secondary">Actividades registradas</p><p className="text-2xl font-medium mt-2">{registrosFiltrados.length}</p><p className="text-xs text-muted mt-1">{primerPeriodo || 'Sin datos'} {ultimaFecha && `→ ${ultimaFecha}`}</p></div><div className="stat-tile"><p className="text-sm text-secondary">Asistentes contabilizados</p><p className="text-2xl font-medium mt-2">{total}</p></div><div className="stat-tile"><p className="text-sm text-secondary">Promedio por actividad</p><p className="text-2xl font-medium mt-2">{actividadPromedio}</p></div></div>
