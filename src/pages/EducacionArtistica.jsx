...
const DISCIPLINAS = { danza: "Danza", teatro: "Teatro", artes_visuales: "Artes visuales", otro: "Otro" };
...
export default function EducacionArtistica() {
  ...
  if (roleLoading || loading) return <div className="module-loading" role="status"><span className="loading-dot" />Cargando Educación Artística...</div>;
  ...
  const insightGeneral = topGrupo?.integrantesCount
    ...
    : "Registra grupos e integrantes para construir una lectura del ministerio artístico.";
  ...
  const disciplinasChartData = { labels: disciplinasConTotal.map((item) => item.label), datasets: [{ label: "Integrantes", data: disciplinasConTotal.map((item) => item.total), backgroundColor: "#e06b35", borderRadius: 4, barThickness: 18 }] };

  return (
    <div className="page-shell">
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="eyebrow">FECP · Educación Artística</p>
      ...
      {!canEdit && <p className="text-sm text-secondary bg-surface-1 rounded p-3">Tienes acceso de consulta. Las altas y modificaciones requieren el permiso de edición de Educación Artística.</p>}
      ...
      <section className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="card chart-card p-5">
          ...
          <h2 className="font-medium mt-1">Integrantes por disciplina</h2>
          <div className="h-56 mt-4">
            {integrantesActivos.length ? <Bar data={disciplinasChartData} options={CHART_OPTIONS} /> : <p className="text-sm text-muted text-center pt-20">Sin integrantes registrados todavía.</p>}
          </div>
        </div>
      </section>
      ...
      <section className="card p-5">
        ...
        <div className="overflow-x-auto mt-4">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-xs text-muted border-b border-border"><th className="py-2">Grupo</th><th className="py-2">Disciplina</th><th className="py-2 text-right">Integrantes</th><th className="py-2 text-right">Asistencia prom.</th></tr></thead>
            <tbody>
      ...
      <section className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          <div className="flex items-start justify-between gap-3">
            <div><p className="eyebrow">Censo</p><h2 className="font-medium mt-1">Integrantes de Educación Artística</h2></div>
