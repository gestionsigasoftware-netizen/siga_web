...
export default function EstacionRefam() {
  ...
  async function load() {
    ...
    const [activosResult, zonasResult, personasResult, amigosResult, gruposResult, estacionesResult] = await Promise.all([
  ...
  async function loadRefamGrupoDetail(grupoId) {
    ...
    if (participantes.length) {
      const { data: asistenciaData } = await supabase.from("refam_asistencia_participante").select("participante_id, asistio").in("participante_id", participantes.map((item) => item.id)).eq("asistio", true);
      const conteo = {};
      (asistenciaData ?? []).forEach((item) => { conteo[item.participante_id] = (conteo[item.participante_id] || 0) + 1; });
  ...
  async function addRefamParticipante(event) {
    event.preventDefault();
    if (!canEdit || !selectedRefamGrupoId || !refamParticipanteForm.sujeto_id || !estacion) return;
    setError(null);
    setSaving(true);
    const payload = {
      congregacion_id: congregacionId,
          ...
          <div>
            {selectedRefamGrupoId ? <div className="flex flex-col gap-4">
              <div>
                ...
                {canEdit && <form onSubmit={addRefamParticipante} className="flex gap-2 mb-2"><select aria-label="Tipo de participante" className="input-field" value={refamParticipanteForm.tipo} onChange={(event) => setRefamParticipanteForm({ tipo: event.target.value, sujeto_id: "" })}><option value="amigo">Amigo</option><option value="persona">Persona</option></select><select aria-label="Participante" required className="input-field flex-1" value={refamParticipanteForm.sujeto_id} onChange={(event) => setRefamParticipanteForm({ ...refamParticipanteForm, sujeto_id: event.target.value })}><option value="">Selecciona...</option>{(refamParticipanteForm.tipo === "amigo" ? amigosDisponibles : personas).map((item) => <option key={item.id} value={item.id}>{item.nombres} {item.apellidos || ""}</option>)}</select><button aria-label="Agregar participante" disabled={saving} className="btn-secondary px-3"><Plus className="w-4 h-4" /></button></form>}
                {refamParticipantes.length ? <div className="divide-y divide-border">{refamParticipantes.map((item) => <div key={item.id} className="py-1.5 text-sm flex justify-between"><span>{item.personas ? `${item.personas.nombres} ${item.personas.apellidos}` : item.amigos?.nombres || "Sin nombre"} <span className="text-xs text-muted">· {item.estado}</span></span><span className="text-xs text-muted">{asistenciaPorParticipante[item.id] || 0} reuniones</span></div>)}</div> : <p className="text-xs text-muted">Sin participantes aún.</p>}
              </div>
              <div>
