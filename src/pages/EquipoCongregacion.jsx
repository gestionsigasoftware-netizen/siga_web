  ...
  return (
    <div className="page-shell">
      ...
      <form onSubmit={inviteUser} className="grid md:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-3 items-end">
        ...
        <label className="text-sm flex items-center gap-1">Acceso web <span className="text-xs text-muted">(opcional)</span><InfoTip texto="Le permite entrar a SIGAP desde el navegador con el nivel de permisos de este perfil. No es lo mismo que la responsabilidad operativa de abajo." /><select className="input-field mt-1.5 w-full" value={profileId} onChange={(event) => setProfileId(event.target.value)}><option value="">Sin acceso web</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.nombre}</option>)}</select></label>
        <label className="text-sm flex items-center gap-1">Responsabilidad operativa <span className="text-xs text-muted">(opcional)</span><InfoTip texto="Asigna a la persona como encargada de capturar datos de un módulo (por ejemplo Ujieres) desde la app móvil, sin necesidad de acceso web." /><select className="input-field mt-1.5 w-full" value={moduleId} onChange={(event) => { setModuleId(event.target.value); setZonaId(''); setCentroId('') }}><option value="">Sin responsabilidad adicional</option>{modules.map((module) => <option key={module.id} value={module.id}>{module.nombre_modulo}</option>)}</select></label>
