...
export default function ReportesOptimizado() {
  ...
  return <div className="page-shell">
    ...
    <section className={`card p-4 grid gap-3 sm:grid-cols-2 xl:items-center ${rolPrincipal?.nivel === 'local' ? 'xl:grid-cols-[auto_1fr_180px]' : 'xl:grid-cols-[auto_1fr_220px_180px]'}`}><div className="flex items-center gap-2 text-sm text-secondary"><Filter className="w-4 h-4" /> Filtros del análisis</div><div className="flex gap-2 flex-wrap">{PERIODS.map(([value, label]) => <button type="button" key={value} onClick={() => setPeriodo(value)} className={`text-xs px-3 py-2 rounded border ${periodo === value ? 'bg-night text-white border-night' : 'border-border text-secondary'}`}>{label}</button>)}</div>{rolPrincipal?.nivel !== 'local' && <select aria-label="Filtrar por congregación" className="input-field min-w-0" value={congregacion} onChange={(event) => setCongregacion(event.target.value)}><option value="todas">Todas las congregaciones</option>{congregations.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select>}<select aria-label="Filtrar por módulo" className="input-field min-w-0" value={modulo} onChange={(event) => setModulo(event.target.value)}><option value="todos">Todos los módulos</option>{byModule.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></section>
