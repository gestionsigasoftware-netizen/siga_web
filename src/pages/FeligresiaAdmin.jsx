...
function FamilyTree({ familyId, families, members, relations, people, canEdit, onOpenPerson, onRefresh, amigos, familiaAmigos }) {
  ...
  async function addAmigoMember(event) { event.preventDefault(); if (!familyId || !amigoMemberForm.amigo_id) return; const result = await supabase.from('familia_amigos').insert({ familia_id: familyId, amigo_id: amigoMemberForm.amigo_id, parentesco: amigoMemberForm.parentesco }); if (result.error) return; setAmigoMemberForm({ amigo_id: '', parentesco: 'otro' }); onRefresh() }
  async function removeAmigoMember(id) { await supabase.from('familia_amigos').delete().eq('id', id); onRefresh() }
