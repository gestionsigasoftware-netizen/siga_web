...
export default function FeligresiaAdmin() {
  ...
  async function renameCommittee(committee) {
    if (!canEdit) { setError('Tu perfil no permite modificar comités.'); return }
    setDialog({ title: 'Editar comité', fields: [{ name: 'nombre', label: 'Nombre', value: committee.nombre, required: true, placeholder: 'Ej: Comité de Evangelismo' }, { name: 'codigo', label: 'Código interno', value: committee.codigo || '', placeholder: 'Ej: COM-EVAN-01 (opcional)', tip: 'Un código o sigla propio de tu congregación para tus archivos internos -- SIGAP no lo usa para nada más.' }, { name: 'descripcion', label: 'Descripción', value: committee.descripcion || '', type: 'textarea', placeholder: 'Qué es este comité, en pocas palabras.' }, { name: 'fecha_inicio', label: 'Fecha de inicio', value: committee.fecha_inicio || '', type: 'date', required: true }, { name: 'fecha_fin', label: 'Fecha de finalización', value: committee.fecha_fin || '', type: 'date', tip: 'Solo si ya sabes cuándo termina este comité. Déjala vacía si es permanente.' }], onSubmit: async (values) => {
  ...
  async function attendPastoralAlert(alert) {
    ...
    setDialog({ title: 'Atender alerta pastoral', message: alert.detalle, fields: [{ name: 'accion', label: 'Acción realizada', value: '', required: true, placeholder: 'Ej: Llamé y programamos una visita el sábado' }, { name: 'fecha', label: 'Fecha realizada', value: hoyBogota(), required: true, type: 'date' }, { name: 'proxima_fecha', label: 'Próximo contacto (opcional)', value: '', type: 'date', tip: 'Si programas una fecha, este seguimiento queda pendiente en tu Agenda hasta esa fecha. Si la dejas vacía, queda completado de una vez.' }, { name: 'notas', label: 'Notas', value: '', type: 'textarea', placeholder: 'Ej: Se mostró interesado/a, dar seguimiento en dos semanas.' }], onSubmit: (values) => saveAlertAttention(alert, values) })
  }
  ...
  return <div className={`flex flex-col gap-6 ${canEdit === false ? 'feligresia-read-only' : ''}`}>
    ...
    {tab === 'traslados' && <section className="flex flex-col gap-4">
      <p className="text-xs text-secondary bg-surface-1 rounded-card px-4 py-2.5">Esta pestaña solo muestra los traslados <strong>ya en curso</strong>, de o hacia tu congregación. Para iniciar uno nuevo, ve a Población, abre la ficha de la persona y usa "Trasladar a otra congregación".</p>
    ...
    {tab === 'personas' && <section className="card overflow-hidden">
      ...
      <div className="px-4 pt-3 flex items-center gap-1.5 text-xs text-muted"><InfoTip texto="'Nuevo bautizado' aparece los primeros días después del bautismo, mientras la persona sigue en seguimiento de Discipulado -- desaparece sola con el tiempo. 'Sugerido: X' es una recomendación automática de comité según la edad, género y estado civil configurados en Configuración → Rangos de edad por comité -- no asigna a nadie solo, es solo una sugerencia para que tú decidas." /><span>Qué significan las etiquetas de cada persona</span></div>
...
function CommitteeCreateForm({ onSubmit, saving, name, setName, code, setCode, type, setType, types, description, setDescription, purpose, setPurpose, start, setStart, end, setEnd, responsible, setResponsible, notes, setNotes, people }) {
  return <form onSubmit={onSubmit} className="card p-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3"><div className="sm:col-span-2"><label className="text-sm">Nombre<input required placeholder="Ej: Comité de Evangelismo" className="input-field mt-1.5" value={name} onChange={(event) => setName(event.target.value)} /></label></div><label className="text-sm flex items-center gap-1">Código interno<InfoTip texto="Un código o sigla que tu congregación use para identificar este comité en sus propios archivos (por ejemplo, en actas o carpetas físicas). Es opcional y tú decides el formato -- SIGAP no lo usa para nada más." /><input placeholder="Ej: COM-EVAN-01 (opcional)" className="input-field mt-1.5 w-full" value={code} onChange={(event) => setCode(event.target.value)} /></label><label className="text-sm flex items-center gap-1">Tipo<InfoTip texto="Clasifica el comité usando los tipos que configuraste en Configuración → Tipos de comité (por ejemplo: Evangelismo, Construcción, Educación). Es opcional y solo sirve para agrupar/filtrar -- si no has configurado tipos, deja 'Sin tipo'." /><select className="input-field mt-1.5 w-full" value={type} onChange={(event) => setType(event.target.value)}><option value="">Sin tipo</option>{types.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="text-sm">Fecha de inicio<input required type="date" className="input-field mt-1.5" value={start} onChange={(event) => setStart(event.target.value)} /></label><label className="text-sm flex items-center gap-1">Fecha de finalización<InfoTip texto="Solo llénala si ya sabes cuándo termina este comité (por ejemplo, uno organizado para un evento puntual). Déjala vacía si es un comité permanente." /><input type="date" className="input-field mt-1.5 w-full" value={end} onChange={(event) => setEnd(event.target.value)} /></label><label className="text-sm">Responsable<select className="input-field mt-1.5" value={responsible} onChange={(event) => setResponsible(event.target.value)}><option value="">Sin responsable</option>{people.filter((person) => person.estado_membresia === 'activo').map((person) => <option key={person.id} value={person.id}>{person.nombres} {person.apellidos}</option>)}</select></label><label className="text-sm sm:col-span-2 flex items-center gap-1">Descripción<InfoTip texto="Qué es este comité, en pocas palabras -- para que cualquiera que lo vea entienda de qué se trata." /><textarea placeholder="Ej: Grupo encargado de organizar las visitas de evangelismo casa por casa en el barrio." className="input-field mt-1.5 min-h-16 w-full" value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="text-sm sm:col-span-2 flex items-center gap-1">Propósito<InfoTip texto="Para qué existe -- el objetivo o resultado que busca este comité (distinto de la descripción, que dice qué ES; el propósito dice PARA QUÉ)." /><textarea placeholder="Ej: Aumentar la asistencia a la iglesia mediante visitas semanales a las familias del sector." className="input-field mt-1.5 min-h-16 w-full" value={purpose} onChange={(event) => setPurpose(event.target.value)} /></label><label className="text-sm sm:col-span-2">Observaciones<textarea placeholder="Notas, acuerdos o decisiones relevantes sobre este comité (opcional)." className="input-field mt-1.5 min-h-16" value={notes} onChange={(event) => setNotes(event.target.value)} /></label><div className="sm:col-span-2 lg:col-span-4 flex justify-end"><button disabled={saving} className="btn-primary"><Plus className="w-4 h-4" />{saving ? 'Guardando...' : 'Crear comité'}</button></div></form>
}
...
function AdminDialog({ dialog, saving, error, close }) {
  ...
  return <div className="fixed inset-0 z-[60] bg-ink/30 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="admin-dialog-title"><form onSubmit={submit} className="w-full max-w-md bg-surface-2 rounded-card shadow-xl p-6"><h2 id="admin-dialog-title" className="font-medium">{dialog.title}</h2>{dialog.message && <p className="text-sm text-secondary mt-2">{dialog.message}</p>}{error && <p role="alert" className="text-sm text-danger bg-danger-bg rounded p-3 mt-3">{error}</p>}{dialog.fields && <div className="flex flex-col gap-3 mt-4">{dialog.fields.map((field) => <label key={field.name} className="text-sm flex items-center gap-1 flex-wrap">{field.label}{field.tip && <InfoTip texto={field.tip} />}{field.type === 'select' ? <select required={field.required} className="input-field mt-1.5 w-full" value={values[field.name]} onChange={(event) => setValues({ ...values, [field.name]: event.target.value })}>{field.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : field.type === 'textarea' ? <textarea required={field.required} placeholder={field.placeholder} className="input-field mt-1.5 min-h-20 w-full" value={values[field.name]} onChange={(event) => setValues({ ...values, [field.name]: event.target.value })} /> : <input required={field.required} type={field.type || 'text'} placeholder={field.placeholder} className="input-field mt-1.5 w-full" value={values[field.name]} onChange={(event) => setValues({ ...values, [field.name]: event.target.value })} />}</label>)}</div>}<div className="flex justify-end gap-2 mt-6"><button type="button" onClick={close} className="btn-secondary">Cancelar</button><button disabled={saving} className="btn-primary">{saving ? 'Guardando...' : dialog.confirmLabel || 'Guardar'}</button></div></form></div>
}
...
function PastoralFollowupPanel({ person, followups, saving, onSubmit, embedded = false }) {
  ...
  return <section className={`${embedded ? '' : 'fixed z-50 right-4 bottom-4 w-[min(24rem,calc(100vw-2rem))] max-h-[75vh] overflow-y-auto bg-surface-2 border border-border rounded-card shadow-xl'} p-4`}><div className="flex items-start justify-between gap-3 mb-3"><div><h2 className="font-medium flex items-center gap-1">Seguimiento pastoral<InfoTip texto="Registra aquí cualquier acompañamiento que hagas con esta persona (llamada, visita, conversación). Queda guardado en su historial y, si programas un próximo contacto, aparece también en 'Agenda de acompañamiento' de la pestaña Seguimiento pastoral." /></h2><p className="text-xs text-secondary mt-1">{person.nombres} {person.apellidos}</p></div><span className="text-xs text-muted">{personFollowups.length} registros</span></div><form onSubmit={onSubmit} className="flex flex-col gap-2 border-b border-border pb-4"><div className="flex items-center gap-1.5"><select name="tipo_alerta" className="input-field text-sm flex-1" defaultValue=""><option value="">Tipo de situación...</option><option value="familia">Familia</option><option value="bautismo">Bautismo</option><option value="asistencia_persona">Asistencia</option><option value="general">General</option></select><InfoTip texto="Clasifica de qué trató el acompañamiento -- usa 'General' si no encaja en ninguna de las otras opciones. Solo es para poder filtrar/agrupar después, no cambia nada más." /></div><input required name="accion" className="input-field text-sm" placeholder="Ej: Llamé para preguntar cómo está la familia" /><div className="grid grid-cols-2 gap-2"><label className="text-xs text-secondary">Fecha realizada<input required name="fecha" type="date" className="input-field text-sm mt-1" defaultValue={hoyBogota()} /></label><label className="text-xs text-secondary flex items-center gap-1">Próximo contacto<InfoTip texto="Si la llenas, este seguimiento queda pendiente en tu Agenda hasta esa fecha. Si la dejas vacía, queda completado de una vez." /><input name="proxima_fecha" type="date" className="input-field text-sm mt-1 w-full" /></label></div><textarea name="notas" className="input-field text-sm min-h-16" placeholder="Ej: Se mostró interesado/a, dar seguimiento en dos semanas." /><button disabled={saving} className="btn-primary justify-center">{saving ? 'Guardando...' : 'Registrar seguimiento'}</button></form><div className="flex flex-col divide-y divide-border">{personFollowups.length ? personFollowups.map((item) => <div key={item.id} className="py-3"><div className="flex justify-between gap-2"><p className="text-sm font-medium">{item.accion}</p><span className={`text-xs ${item.estado === 'completado' ? 'text-success' : item.estado === 'cancelado' ? 'text-muted' : 'text-accent'}`}>{item.estado || 'pendiente'}</span></div><p className="text-xs text-muted mt-1">{item.fecha}{item.proxima_fecha ? ` · Próximo: ${item.proxima_fecha}` : ''}{item.tipo_alerta ? ` · ${ALERT_TYPE_LABELS[item.tipo_alerta] || item.tipo_alerta}` : ''}</p>{item.notas && <p className="text-xs text-secondary mt-1">{item.notas}</p>}</div>) : <p className="text-xs text-muted py-4">Aún no hay seguimientos registrados.</p>}</div></section>
}
...
function MembershipMovementsPanel({ person, movimientosMembresia, saving, onSubmit, trasladoBusqueda, trasladoResultados, trasladoDestinoId, setTrasladoDestinoId, trasladoObservaciones, setTrasladoObservaciones, savingTraslado, onBuscarDestino, onIniciarTraslado, embedded = false }) {
  ...
  return <section className={`${embedded ? '' : 'fixed z-50 right-4 bottom-4 w-[min(24rem,calc(100vw-2rem))] max-h-[75vh] overflow-y-auto bg-surface-2 border border-border rounded-card shadow-xl'} p-4`}>
    ...
    <div className="flex flex-col gap-2 mt-3 border-b border-border pb-4">
      ...
      <input className="input-field text-sm" placeholder="Ej: Se muda por trabajo a partir de enero (opcional)" value={trasladoObservaciones} onChange={(event) => setTrasladoObservaciones(event.target.value)} />
