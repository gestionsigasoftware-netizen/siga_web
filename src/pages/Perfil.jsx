...
export default function Perfil() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      <div><p className="eyebrow">Cuenta</p><h1 className="section-title">Mi perfil</h1><p className="text-sm text-secondary mt-1">Administra tu acceso y consulta tus permisos dentro de SIGA.</p></div>
      <section className="card p-6 flex items-center gap-4"><div className="w-14 h-14 rounded-full bg-ink text-white flex items-center justify-center text-xl font-semibold">{(user?.email?.[0] || 'U').toUpperCase()}</div><div><h2 className="font-medium">{user?.user_metadata?.full_name || 'Usuario SIGA'}</h2><p className="text-sm text-secondary flex items-center gap-2 mt-1"><Mail className="w-4 h-4" /> {user?.email}</p></div></section>
