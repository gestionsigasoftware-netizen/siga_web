...
export default function Perfil() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      <section className="card p-6"><div className="flex items-center gap-3 mb-5"><ShieldCheck className="w-5 h-5 text-success" /><div><h2 className="font-medium">Permisos asignados</h2><p className="text-xs text-secondary mt-1">Roles activos de esta cuenta.</p></div></div><div className="flex gap-2 flex-wrap">{roles.length ? roles.map((role) => <span key={role.id} className="text-xs bg-accent-bg text-accent-dark rounded px-3 py-2">{NIVEL_LABEL[role.nivel] || role.nivel} · {describirAlcance(role)}</span>) : <p className="text-sm text-muted">No hay roles activos.</p>}</div></section>
