...
import { LayoutDashboard, ClipboardPlus, Users, CheckSquare, Settings, LogOut, UserRound, Layers3, FileBarChart2, HeartHandshake, ClipboardList, UserPlus, ArrowRightLeft, Menu, X } from 'lucide-react'
...
export default function Sidebar() {
  ...
  const items = [
    ...
    { to: '/auditoria-feligresia', label: 'Auditoría de Feligresía', icon: ClipboardList, show: nivel !== 'local' || rolLocal === 'pastor' },
    ...
    { to: '/configuracion', label: 'Configuración local', icon: Settings, show: puedeConfigurar },
    { to: '/configuracion-sistema', label: 'Preferencias personales', icon: Settings, show: true },
  ].filter((i) => i.show)
  ...
  return (
    <aside className="sidebar-shell w-full md:w-[248px] flex-shrink-0 md:h-screen md:fixed md:left-0 md:top-0 flex flex-col p-3 md:p-4">
      <div className="flex items-center justify-between gap-3 px-2 pb-2 md:pb-5 pt-1">
        ...
        <button type="button" onClick={() => setMobileOpen((current) => !current)} className="md:hidden p-2 text-white/75 hover:text-white" aria-label={mobileOpen ? 'Cerrar navegación' : 'Abrir navegación'} aria-expanded={mobileOpen}>
      ...
      <nav className="flex flex-row flex-wrap md:flex-col gap-1.5 md:overflow-y-auto">
        {items.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to} end onClick={() => setMobileOpen(false)} className={({ isActive }) => `navbtn ${isActive ? 'navbtn-active' : ''}`}>
